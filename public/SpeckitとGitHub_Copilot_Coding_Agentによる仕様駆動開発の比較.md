...
| **マルチデバイス** | ×（ローカルのみ、スマホで指示できない） | ◎（どこからでも可能、スマホから指示できる） |
| **仕様の型** | テンプレート・型あり | 自由形式 |
| **学習曲線** | 中（型の理解が必要） | 低（自然言語で指示可能） |
| **初心者向け** | ◎（型が指針になる、仕様駆動に慣れてない人でも進める指針を得られる） | ○（自由度が高い分迷いも、まとめ方の指針がない） |
| **柔軟性** | △（型に制約される、仕様の理解や調整に時間を使う） | ◎（自由に指示可能、型がないので自由にできる） |
| **中断・再開** | △ | ◎（途中で辞めたり、人間との連携はしやすい） |
| **人間との連携** | △ | ◎ |
| **複数変更同時実行** | △ | ◎（複数の変更を同時に指示できる） |
| **CI/CD統合** | △ | ◎（CIネイティブに考えやすい） |
| **トラブル対応** | 難（環境依存、変な動きをした時の対応方法に悩む） | 中（一般的な問題が多い） |
